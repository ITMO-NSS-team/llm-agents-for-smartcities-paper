...
def parse_function_names_from_check_llm_answer_for_metrics(
    llm_res: str,
) -> List:
...
def parse_function_names_from_llm_answer(llm_res: str) -> List:
    res = []
    functions = [
        "service_accessibility_pipeline",
...
def check_pipeline(q: str, r_f: List[str]) -> List[str]:
    ...
    checked_res_funcs = parse_function_names_from_check_llm_answer(
        llm_check_res
    )
