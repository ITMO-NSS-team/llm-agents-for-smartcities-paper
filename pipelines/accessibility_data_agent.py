...
def get_relevant_api_function_from_llm(
    model_url: str, tools: List, question: str
) -> str:
    params = {
        "messages": [
            {
...
def parse_function_names_from_llm_answer(llm_res: str) -> List[str]:
    ...
    correct_pred_funcs = set(
        map(lambda x: get_nearest_levenstein(x, functions), predicted_funcs)
    )
...
def retrieve_context_from_api(c: list, r_f: list) -> str:
    ...
    try:
        ...
        logger.error(
            f"Accessibility agent: Could NOT retrieve context from API: {e}"
        )
    return context
...
def service_accessibility_pipeline(
    ...
    if t_type == "city":
        ...
        t_type is None and t_id is not None and type(coordinates[0]) is not int
    ):
...
def run_pipeline_on_local_data(question: str, table: str):
    input_data = {
        ...
        "get_general_stats_districts_mo": read_json_file(
            Path(
                path_to_data,
                "summary_tables",
                "get_general_stats_districts_mo.json",
            )
        ),
        "get_general_stats_block": read_json_file(
            Path(
                path_to_data, "summary_tables", "get_general_stats_block.json"
            )
        ),
        "get_general_stats_education": read_json_file(
            Path(
                path_to_data,
                "summary_tables",
                "get_general_stats_education.json",
            )
        ),
        "get_general_stats_healthcare": read_json_file(
            Path(
                path_to_data,
                "summary_tables",
                "get_general_stats_healthcare.json",
            )
        ),
        ...
        "get_general_stats_sports": read_json_file(
            Path(
                path_to_data, "summary_tables", "get_general_stats_sports.json"
            )
        ),
        "get_general_stats_services": read_json_file(
            Path(
                path_to_data,
                "summary_tables",
                "get_general_stats_services.json",
            )
        ),
    }
    ...
    if table == "Общий контекст – общая таблица сводка по городу":
        ...
        table
        == "Территориальный контекст – сводка по выбранному району или МО"
    ):
        res_funcs.append("get_general_stats_districts_mo")
...
def check_choice_correctness(
    question: str, answer: str | List[str], tools: List
):
    ...
    user_message = (
        f"[Instruction]: You are given question, descriptions functions and an answer"
...
def run_test_full_pipeline_with_local_data():
    ...
    print(
        ...
        round(correct_answers / all_questions * 100, 2),
    )
