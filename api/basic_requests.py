...
def get_house_coordinates(city_id: int, address: str) -> List:
    houses = Api.EndpointsCity.houses(
        city=city_id,
...
def get_demand_within(city, service_id, area_coordinates, year):
    ...
    sum = round(
        sum(
            map(
                lambda house: house["properties"]["supplyed_demands_within"],
                houses,
            )
        )
    )
    avg = sum / len
    return (
        f"Количество человек, обеспеченных сервисом в заданном диапазоне доступности: {sum},"
...
def get_demand_without(city, service_id, area_coordinates, year):
    ...
    sum = round(
        sum(
            map(
                lambda house: house["properties"]["supplyed_demands_without"],
                houses,
            )
        )
    )
    avg = sum / len
    return (
        f"Количество человек, обеспеченных сервисом вне заданного диапазона доступности: {sum},"
...
def get_capacity_within(city, service_id, area_coordinates, year):
    ...
    sum = round(
        sum(
            map(
                lambda house: house["properties"]["carried_capacity_within"],
                houses,
            )
        )
    )
    avg = sum / len
    return (
        f"Количество мест, занятых людьми из заданного диапазона доступности: {sum},"
...
def get_capacity_without(city, service_id, area_coordinates, year):
    ...
    sum = round(
        sum(
            map(
                lambda house: house["properties"]["carried_capacity_without"],
                houses,
            )
        )
    )
    avg = sum / len
    return (
        f"Количество мест, занятых людьми не из заданного диапазона доступности: {sum},"
